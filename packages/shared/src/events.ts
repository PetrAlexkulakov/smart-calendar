...
/**
 * Базовая проверка правила повторения: обязательный FREQ из списка
 * допустимых частот и только разрешённые символы.
 *
 * Полный разбор правила намеренно оставлен бэкенду. Библиотека rrule
 * поставляется двумя сборками — ESM с именованными экспортами и CJS,
 * где доступен только default, — и один и тот же импорт не работает
 * сразу в Vite и в Node. Тащить её в общий пакет ради валидации формы
 * не стоит: сервер всё равно перепроверит правило разбором.
 *
 * SECONDLY и MINUTELY не входят в список сознательно: минутное правило
 * на годовом окне — это полмиллиона вхождений.
 */
const rruleSchema = z
  .string()
  .max(500)
  .regex(
    /^(?=.*\bFREQ=(YEARLY|MONTHLY|WEEKLY|DAILY|HOURLY)\b)[A-Za-z0-9=;,:+\-/]*$/,
